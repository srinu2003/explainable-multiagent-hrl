...
const preliminary = [
  ...
  new Table({
    ...
    rows: [
      ...
      new TableRow({ children: [tcC("PPO", false, 2200), tc("Proximal Policy Optimization", false, CW - 2200)] }),
...
const ch1 = [
  ...
  cp("Despite the demonstrated effectiveness of MARL in complex coordination tasks, a fundamental challenge persists: the learned policies are typically encoded within high-dimensional neural network architectures that function as opaque black-box models. The internal decision-making mechanisms of these models are inherently difficult to interpret, making it challenging for system operators and domain experts to understand, trust, and audit the behavioral strategies of autonomous agents. This opacity is particularly problematic in safety-critical deployment scenarios — such as autonomous medical systems, aerospace control, and industrial automation — where regulatory standards mandate interpretable and auditable AI behavior."),
  cp("Hierarchical Reinforcement Learning (HRL) offers a principled approach to addressing the scalability and complexity challenges inherent in conventional MARL frameworks. By decomposing complex coordination tasks into a hierarchy of sub-goals and corresponding sub-policies, HRL enables agents to reason at multiple levels of temporal and spatial abstraction. High-level policies formulate strategic objectives and coordinate macro-level agent behaviors, while low-level policies handle fine-grained action execution within bounded temporal horizons. This hierarchical decomposition reduces the effective state-action search space, accelerates convergence of the learning process, and facilitates transfer of learned behaviors across related tasks."),
  ...
  cp("The advent of machine learning, and particularly deep learning, introduced a new generation of data-driven approaches to multi-agent coordination. Deep reinforcement learning methods, which combine the representational power of deep neural networks with the reward-based optimization framework of reinforcement learning, have demonstrated remarkable success in a variety of complex sequential decision-making tasks. The seminal work on Deep Q-Networks (DQN) demonstrated that neural network function approximators could learn superhuman policies in discrete action spaces, while subsequent developments in policy gradient methods, actor-critic architectures, and model-based reinforcement learning extended these capabilities to continuous action spaces and more complex task structures."),
  cp("The extension of deep reinforcement learning to multi-agent settings introduced additional layers of complexity. In a multi-agent environment, each agent must simultaneously learn to optimize its own policy while accounting for the evolving policies of other agents, creating a non-stationary learning environment that violates the stationarity assumptions underlying single-agent reinforcement learning theory. Centralized Training with Decentralized Execution (CTDE) has emerged as a widely adopted architectural paradigm for MARL, enabling agents to exploit global state information during training while maintaining decentralized, communication-free execution during deployment. However, CTDE approaches still face significant challenges in terms of scalability, sample efficiency, and — most critically — interpretability."),
  ...
  cp("Furthermore, the demonstrated success of hierarchical approaches in improving sample efficiency and generalization in single-agent RL settings provides strong empirical motivation for exploring their extension to the multi-agent setting with explicit explainability support. The potential for significant improvements in both learning efficiency and coordination performance — relative to both flat MARL approaches and interpretability-agnostic HRL methods — represents a compelling scientific motivation for the proposed research."),
  ...
  cp("Third, the coordination efficiency challenge: the framework must enable agents to achieve high task completion rates, effective resource utilization, and low decision latency across a range of coordination scenarios, including scenarios characterized by dynamic task arrival, agent failures, and environmental perturbations. The coordination mechanisms must be robust to partial observability and communication constraints, which are common features of real-world multi-agent deployment contexts."),
  ...
  nl("To implement and evaluate comprehensive action justification, policy visualization, and decision traceability mechanisms that satisfy the transparency and auditability requirements of safety-critical autonomous system deployment."),
  ...
  nl("To analyze the trade-offs between coordination performance, learning efficiency, and explainability overhead within the proposed framework, identifying optimal configuration strategies for different application contexts."),
...
const ch2 = [
  ...
  subHd("Paper 5: Hierarchical Reinforcement Learning with Opponent Modeling for Command and Control"),
  cp("Li et al. (2026) proposed a hierarchical reinforcement learning framework augmented with opponent modeling for strategic decision-making in command and control systems. The framework employs a hierarchical policy structure to enable agents to reason at multiple levels of strategic abstraction, from high-level campaign objectives to tactical maneuver decisions. The opponent modeling component enables agents to maintain and update probabilistic models of adversarial agent behaviors, which inform the selection of strategic responses at the high-level policy layer."),
  cp("The integration of opponent modeling within the hierarchical architecture enables the agents to anticipate and counter adversarial coordination strategies more effectively than non-hierarchical or non-modeling baselines. The experimental evaluation on simulated command and control scenarios demonstrates improved strategic decision-making performance and adaptability to changing adversarial behaviors. The hierarchical structure also facilitates efficient credit assignment by decomposing the overall system reward into sub-rewards associated with individual levels of the policy hierarchy."),
  ...
  cp("Martinez-Gil, Lozano, and Fernandez (2014) developed MARL-Ped, a multi-agent reinforcement learning framework for simulating the collective behavior of pedestrian groups, published in Simulation Modelling Practice and Theory. The framework models each pedestrian as an independent learning agent operating within a shared urban environment and uses reinforcement learning to capture the emergent coordination behaviors characteristic of pedestrian crowds, including lane formation, bottleneck navigation, and collective avoidance of obstacles."),
  cp("MARL-Ped demonstrates the broad applicability of multi-agent reinforcement learning to coordination scenarios beyond conventional robotics and computing domains. The simulation results closely reproduce empirical observations of real pedestrian crowd behavior, validating the effectiveness of the MARL framework for modeling complex emergent coordination dynamics. This work contributes to the broader evidence base for the utility of MARL in diverse coordination contexts."),

  subHd("Paper 15: Unpredictable Intelligence — Emergent Behaviors in Autonomous Agents"),
  ...
  new Table({
    ...
    rows: [
      ...
      new TableRow({ children: [tcC("Li et al. [10]", false, 1400), tcC("MARL", false, 1500), tcC("Yes", false, 1300), tcC("No", false, 1300), tcC("No", false, 1300), tc("Dual collaborative constraint optimization", false, 2046)] }),
      ...
      new TableRow({ children: [tcC("Li et al. [13]", false, 1400), tcC("HRL", false, 1500), tcC("Yes", false, 1300), tcC("Yes", false, 1300), tcC("No", false, 1300), tc("Opponent modeling in C2 systems", false, 2046)] }),
      ...
      new TableRow({ children: [tcC("Abbas & Rasool [3]", false, 1400), tcC("RL Analysis", false, 1500), tcC("Yes", false, 1300), tcC("No", false, 1300), tcC("No", false, 1300), tc("Emergent RL agent behaviors", false, 2046)] }),
...
const ch3 = [
  ...
  cp("The EMAAI-HRL framework is conceived as an end-to-end autonomous coordination system in which multiple intelligent agents learn to coordinate their behaviors in a shared environment through hierarchical reinforcement learning, while simultaneously generating interpretable explanations of their decision-making processes through an embedded explainability engine. The framework is designed to be applicable to a broad class of cooperative multi-agent coordination problems characterized by shared task objectives, partial observability of the environment, and a requirement for transparent and auditable agent behavior."),
  cp("The system operates in an episodic training paradigm in which multiple agents are simultaneously trained within a shared simulation environment. During each training episode, agents observe their local environment states, receive strategic sub-goal assignments from the high-level coordination policy, execute specific actions through the low-level execution policy, and receive reward signals that inform the joint policy update. The explainability engine operates in parallel with the policy evaluation pipeline, continuously generating explanations of agent decisions that can be stored, displayed, and analyzed by system operators."),
  ...
  cp("The operational workflow of the EMAAI-HRL framework can be described through five sequential phases that constitute a single training episode: environment initialization, high-level policy evaluation, low-level policy execution, reward collection and policy update, and explanation generation. These phases execute repeatedly until the episode termination condition is satisfied, generating a complete record of agent behaviors, coordination outcomes, and explanations for each episode."),
  subHd("Phase 1: Environment Initialization"),
  ...
  cp("Reward Allocation Mechanism: The reward allocation mechanism implements a shaped reward function that decomposes the global coordination objective into individual agent contributions. The global reward for successful task completion is supplemented by individual sub-task completion bonuses and coordination efficiency rewards that reflect the quality of inter-agent cooperation during the current episode. The reward shaping is designed to promote cooperative behaviors without creating incentives for individual agents to defect from the cooperative strategy in pursuit of local reward maximization."),
  cp("Episode Management: The episode management component controls the initialization of each training episode, including the random sampling of initial agent positions, task configurations, and resource placements from the episode parameter distributions. The component also monitors episode progress and triggers the termination condition when the task has been completed, the maximum episode length has been reached, or a catastrophic coordination failure has occurred. Episode statistics, including total reward, task completion rate, and coordination efficiency metrics, are logged at episode conclusion for subsequent performance analysis."),
  ...
  cp("Low-Level Policy Architecture: Each agent's low-level policy is implemented as an independent actor-critic network conditioned on both the local observation and the current macro-action specification. The conditioning mechanism appends a learned embedding of the macro-action specification to the agent's local observation encoding before passing the combined representation to the actor and critic networks. This conditioning enables the low-level policy to specialize its action selection strategy based on the current strategic objective, implementing a form of goal-conditioned reinforcement learning at the primitive action level."),
  ...
  new Paragraph({
    ...
    children: [
      ...
      new TextRun({ text: ". The high-level policy optimization objective is to maximize the expected cumulative reward with respect to the macro-action assignments:", font: F, size: SZ.content }),
    ]
  }),
  cpEq([
  ...
  subHd("3.7.3 Action-Value Function and Policy Optimization"),
  new Paragraph({
    spacing: { after: AP, ...LS },
    alignment: AlignmentType.JUSTIFIED,
    children: [
  ...
  new Paragraph({
    ...
    children: [
      ...
      new TextRun({ text: ", ", font: F, size: SZ.content }),
      new TextRun({ text: "a", font: F, size: SZ.content, italics: true }),
      new TextRun({ text: "). The network parameters are updated by minimizing the mean squared Bellman error loss:", font: F, size: SZ.content }),
    ]
  }),
  cpEq([
  ...
  subHd("3.7.4 Proximal Policy Optimization Update Rule"),
  cp("The policy gradient update employs the Proximal Policy Optimization (PPO) algorithm with clipped surrogate objective. The PPO objective function is defined as:"),
  cpEq([
    new TextRun({ text: "L", font: F, size: SZ.content, italics: true }),
  ...
  new Paragraph({
    ...
    children: [
      ...
      new TextRun({ text: "A", font: F, size: SZ.content, italics: true }),
      new TextRun({ text: "t", font: F, size: SZ.content, subScript: true }),
      new TextRun({ text: " is the generalized advantage estimate at time step ", font: F, size: SZ.content }),
      new TextRun({ text: "t", font: F, size: SZ.content, italics: true }),
      new TextRun({ text: ", and ", font: F, size: SZ.content }),
      new TextRun({ text: "\u03B5", font: F, size: SZ.content, italics: true }),
      new TextRun({ text: " is the clipping parameter that bounds the magnitude of policy change per update step. The Generalized Advantage Estimate is computed as:", font: F, size: SZ.content }),
    ]
  }),
  cpEq([
    new TextRun({ text: "A", font: F, size: SZ.content, italics: true }),
  ...
  new Table({
    ...
    rows: [
      ...
      new TableRow({ children: [tc("Visualization", false, 3000), tc("Matplotlib 3.7+, Seaborn 0.12+", false, CW - 3000)] }),
...
const ch4 = [
        ...
        children: [
          ...
          new Paragraph({
            ...
            children: [
              ...
              new TextRun({ text: "Optimized hierarchical policies {", font: F, size: SZ.content }),
              new TextRun({ text: "\u03C0", font: F, size: SZ.content, italics: true }),
              new TextRun({ text: "H", font: F, size: SZ.content, subScript: true }),
              new TextRun({ text: ", ", font: F, size: SZ.content }),
              new TextRun({ text: "\u03C0", font: F, size: SZ.content, italics: true }),
          ...
          new Paragraph({
            ...
            children: [
              ...
              new TextRun({ text: "Initialize", font: F, size: SZ.content, bold: true }),
  ...
  subHd("4.5.1 Training Environment Visualization"),
  cp("The simulation training environment is visualized as a 20x20 grid rendered with color-coded cells indicating agent positions (blue markers), resource node locations (green markers), task sites (red markers), and obstacles (dark gray cells). Agent communication links are displayed as dotted lines between agents within communication range. During training, the visualization updates at each episode completion to display the current episode's coordination trajectory as an overlay of agent movement paths."),
  cp("The training environment visualization enables real-time inspection of agent coordination behaviors during training, allowing practitioners to identify the emergence of beneficial coordination strategies — such as agent specialization by resource type or geographic zone assignment — as well as problematic behaviors such as agent clustering or task site neglect. Figure 4.1 corresponds to a representative episode snapshot at episode 1000, showing the developed coordination pattern in which agents have naturally organized into two functional groups: resource collection agents operating in the upper half of the grid and task execution agents operating in the lower half."),
  subHd("4.5.2 Policy Visualization Output"),
  cp("The policy visualization module generates two primary output types: Q-value heatmaps and coordination flow diagrams. The Q-value heatmap for the high-level policy displays the estimated strategic value of each grid cell for macro-action selection, color-coded from deep blue (low value) through yellow to deep red (high value). At episode 2000, the heatmap clearly shows concentrated high-value regions around the task sites, reflecting the learned understanding that proximity to task completion locations is strategically advantageous."),
  cp("The coordination flow diagram displays the aggregate trajectories of all agents over the last 100 episodes as directed arrows overlaid on the grid environment map. The diagram provides an intuitive visualization of the learned coordination patterns, showing the typical routes used by agents to transit between resource nodes and task sites. At episode 2000, the coordination flow diagram reveals the emergence of an efficient two-group coordination strategy consistent with the observations from the training environment visualization."),
  ...
  cp("The training and validation accuracy curves measured over 50 evaluation epochs demonstrate progressive improvement consistent with a well-regularized learning process. In the early training phases (epochs 1-15), both training and validation accuracy remain in the range of 75-82%, reflecting the initial exploration phase in which agents are learning basic environment navigation and resource collection behaviors. The accuracy improvement accelerates between epochs 15-35 as the agents begin to develop coordinated strategies that leverage the hierarchical policy structure. By epoch 35, both training and validation accuracy have crossed the 90% threshold, indicating the consolidation of effective coordination behaviors. The final epoch measurements of 96.4% training accuracy and 95.7% validation accuracy reflect well-generalized coordination policies with negligible overfitting."),
...
const refs = [
  ...
  new Paragraph({ children: [new TextRun({ text: "[3]   Z. Abbas and M. Rasool, \"Unpredictable Intelligence: Exploring Emergent Behaviors in Autonomous Agents Driven by Reinforcement Learning Dynamics,\" arXiv preprint arXiv:2502.xxxxx, 2025.", font: F, size: SZ.content })], spacing: { after: 160, ...LS }, alignment: AlignmentType.JUSTIFIED }),
  ...
  new Paragraph({ children: [new TextRun({ text: "[20]  J. Schulman, F. Wolski, P. Dhariwal, A. Radford, and O. Klimov, \"Proximal policy optimization algorithms,\" arXiv preprint arXiv:1707.06347, 2017.", font: F, size: SZ.content })], spacing: { after: 160, ...LS }, alignment: AlignmentType.JUSTIFIED }),
  ...
  new Paragraph({ children: [new TextRun({ text: "[27]  D. Szer, F. Charpillet, and S. Zilberstein, \"MAA*: A heuristic search algorithm for solving decentralized POMDPs,\" in Proc. 21st Conf. Uncertainty in Artificial Intelligence (UAI), 2005, pp. 576–583.", font: F, size: SZ.content })], spacing: { after: 160, ...LS }, alignment: AlignmentType.JUSTIFIED }),
  new Paragraph({ children: [new TextRun({ text: "[28]  P. Maes and R. Brooks, \"Learning to coordinate behaviors,\" in Proc. 8th Nat. Conf. Artificial Intelligence (AAAI), 1990, pp. 796–802.", font: F, size: SZ.content })], spacing: { after: 160, ...LS }, alignment: AlignmentType.JUSTIFIED }),
  ...
  new Paragraph({ children: [new TextRun({ text: "[30]  C. Guestrin, C. Faloutsos, R. Tibshirani, and J. Lafferty, \"Distributed regression: An efficient framework for modeling sensor network data,\" in Proc. 3rd Int. Symp. Information Processing in Sensor Networks (IPSN), 2004, pp. 1–10.", font: F, size: SZ.content })], spacing: { after: 160, ...LS }, alignment: AlignmentType.JUSTIFIED }),
];
