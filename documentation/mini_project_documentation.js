...
class Bookmark extends DocxBookmark {
  constructor(options) {
    super(options);
    const linkId = ++bookmarkCounter;
...
function getBookmarkId(text) {
  let clean = text.toLowerCase()
    .replace(/^chapter\s+\d+[:\s]*/g, "") // strip chapter labels
    .replace(/[^a-z0-9]/g, "_")
    .replace(/_+/g, "_")
    .replace(/^_+|_+$/g, "");
  return "bm_" + clean.substring(0, 35);
}
...
const cpCite = (text, citeList = []) => {
  ...
  let hasPeriod = false;
  if (text.endsWith(".")) {
    mainText = text.substring(0, text.length - 1);
    hasPeriod = true;
  }
  children.push(new TextRun({ text: mainText + " ", font: F, size: SZ.content }));
  for (let i = 0; i < citeList.length; i++) {
    const num = citeList[i];
    children.push(new TextRun({ text: `[${num}]`, font: F, size: SZ.content }));
    if (i < citeList.length - 1) {
      children.push(new TextRun({ text: ", ", font: F, size: SZ.content }));
    }
  }
  if (hasPeriod) {
    children.push(new TextRun({ text: ".", font: F, size: SZ.content }));
...
const cpTableCaption = (label, text) => {
  const bookmarkId = "tbl_" + label.toLowerCase().replace(/[^a-z0-9_]/g, "_").replace(/_+/g, "_");
  return new Paragraph({
    children: [
      new Bookmark({
        id: bookmarkId,
        children: [
          new TextRun({ text: label + ": " + text, font: F, size: SZ.content, bold: true })
        ]
      })
    ],
    spacing: { after: AP, ...LS },
  ...
  children: [new Paragraph({
    children: [
      new InternalHyperlink({
        children: [
          new TextRun({ text, font: F, size: SZ.content, bold: isHdr, color: isHdr ? "000000" : "0000FF", underline: !isHdr })
        ],
        anchor: bookmarkId
      })
    ],
...
const tcPageRef = (bookmarkId, isHdr, w) => new TableCell({
  borders: bdrs,
  width: { size: w, type: WidthType.DXA },
  margins: margs,
  shading: isHdr ? { fill: "CCCCCC", type: ShadingType.CLEAR } : undefined,
  children: [new Paragraph({
    children: [
      new PageReference(bookmarkId, { hyperlink: false })
    ],
    alignment: AlignmentType.CENTER,
    spacing: { after: 0 },
  })],
  ...
  children.push(new Paragraph({
    children: [
      new Bookmark({
        id: bookmarkId,
        children: [
          new TextRun({ text: `Figure ${figNum}: ${title}`, font: F, size: 22, italics: true })
        ]
      })
    ],
    spacing: { after: 300 },
...
const nl = (text, instance = 1) => new Paragraph({
...
const preliminary = [
  ...
  new Table({
    ...
    rows: [
      ...
      new TableRow({ children: [tc("Declaration", false, CW - 1200), tcPageRef(getBookmarkId("Declaration"), false, 1200)] }),
      new TableRow({ children: [tc("Certificate", false, CW - 1200), tcPageRef(getBookmarkId("Certificate"), false, 1200)] }),
      new TableRow({ children: [tc("Acknowledgment", false, CW - 1200), tcPageRef(getBookmarkId("Acknowledgment"), false, 1200)] }),
      new TableRow({ children: [tc("Abstract", false, CW - 1200), tcPageRef(getBookmarkId("Abstract"), false, 1200)] }),
      new TableRow({ children: [tc("Table of Contents", false, CW - 1200), tcPageRef(getBookmarkId("Table of Contents"), false, 1200)] }),
      new TableRow({ children: [tc("List of Figures", false, CW - 1200), tcPageRef(getBookmarkId("List of Figures"), false, 1200)] }),
      new TableRow({ children: [tc("List of Tables", false, CW - 1200), tcPageRef(getBookmarkId("List of Tables"), false, 1200)] }),
      new TableRow({ children: [tc("List of Abbreviations", false, CW - 1200), tcPageRef(getBookmarkId("List of Abbreviations"), false, 1200)] }),
      new TableRow({ children: [tc("CHAPTER 1: INTRODUCTION", true, CW - 1200), tcPageRef(getBookmarkId("CHAPTER 1: INTRODUCTION"), true, 1200)] }),
      new TableRow({ children: [tc("1.1  Introduction", false, CW - 1200), tcPageRef(getBookmarkId("1.1  Introduction"), false, 1200)] }),
      new TableRow({ children: [tc("1.2  Background of the Problem", false, CW - 1200), tcPageRef(getBookmarkId("1.2  Background of the Problem"), false, 1200)] }),
      new TableRow({ children: [tc("1.3  Motivation", false, CW - 1200), tcPageRef(getBookmarkId("1.3  Motivation"), false, 1200)] }),
      new TableRow({ children: [tc("1.4  Problem Statement", false, CW - 1200), tcPageRef(getBookmarkId("1.4  Problem Statement"), false, 1200)] }),
      new TableRow({ children: [tc("1.5  Proposed Solution", false, CW - 1200), tcPageRef(getBookmarkId("1.5  Proposed Solution"), false, 1200)] }),
      new TableRow({ children: [tc("1.6  Objectives", false, CW - 1200), tcPageRef(getBookmarkId("1.6  Objectives"), false, 1200)] }),
      new TableRow({ children: [tc("1.7  Scope of the Work", false, CW - 1200), tcPageRef(getBookmarkId("1.7  Scope of the Work"), false, 1200)] }),
      new TableRow({ children: [tc("1.8  Research Methodology", false, CW - 1200), tcPageRef(getBookmarkId("1.8  Research Methodology"), false, 1200)] }),
      new TableRow({ children: [tc("1.9  Organization of the Report", false, CW - 1200), tcPageRef(getBookmarkId("1.9  Organization of the Report"), false, 1200)] }),
      new TableRow({ children: [tc("1.10 Summary", false, CW - 1200), tcPageRef(getBookmarkId("1.10 Summary"), false, 1200)] }),
      new TableRow({ children: [tc("CHAPTER 2: LITERATURE SURVEY", true, CW - 1200), tcPageRef(getBookmarkId("CHAPTER 2: LITERATURE SURVEY"), true, 1200)] }),
      new TableRow({ children: [tc("2.1  Introduction", false, CW - 1200), tcPageRef(getBookmarkId("2.1  Introduction"), false, 1200)] }),
      new TableRow({ children: [tc("2.2  Existing Systems", false, CW - 1200), tcPageRef(getBookmarkId("2.2  Existing Systems"), false, 1200)] }),
      new TableRow({ children: [tc("2.3  Research Papers Review", false, CW - 1200), tcPageRef(getBookmarkId("2.3  Research Papers Review"), false, 1200)] }),
      new TableRow({ children: [tc("2.4  Comparative Study", false, CW - 1200), tcPageRef(getBookmarkId("2.4  Comparative Study"), false, 1200)] }),
      new TableRow({ children: [tc("2.5  Limitations of Existing Methods", false, CW - 1200), tcPageRef(getBookmarkId("2.5  Limitations of Existing Methods"), false, 1200)] }),
      new TableRow({ children: [tc("2.6  Research Gap", false, CW - 1200), tcPageRef(getBookmarkId("2.6  Research Gap"), false, 1200)] }),
      new TableRow({ children: [tc("2.7  Summary", false, CW - 1200), tcPageRef(getBookmarkId("2.7  Summary"), false, 1200)] }),
      new TableRow({ children: [tc("CHAPTER 3: PROPOSED METHODOLOGY", true, CW - 1200), tcPageRef(getBookmarkId("CHAPTER 3: PROPOSED METHODOLOGY"), true, 1200)] }),
      new TableRow({ children: [tc("3.1  Introduction", false, CW - 1200), tcPageRef(getBookmarkId("3.1  Introduction"), false, 1200)] }),
      new TableRow({ children: [tc("3.2  Proposed System", false, CW - 1200), tcPageRef(getBookmarkId("3.2  Proposed System"), false, 1200)] }),
      new TableRow({ children: [tc("3.3  Working Principle", false, CW - 1200), tcPageRef(getBookmarkId("3.3  Working Principle"), false, 1200)] }),
      new TableRow({ children: [tc("3.4  Proposed System Architecture", false, CW - 1200), tcPageRef(getBookmarkId("3.4  Proposed System Architecture"), false, 1200)] }),
      new TableRow({ children: [tc("3.5  Module Descriptions", false, CW - 1200), tcPageRef(getBookmarkId("3.5  Module Descriptions"), false, 1200)] }),
      new TableRow({ children: [tc("3.6  System Design", false, CW - 1200), tcPageRef(getBookmarkId("3.6  System Design"), false, 1200)] }),
      new TableRow({ children: [tc("3.7  Mathematical Model", false, CW - 1200), tcPageRef(getBookmarkId("3.7  Mathematical Model"), false, 1200)] }),
      new TableRow({ children: [tc("3.8  Technologies Used", false, CW - 1200), tcPageRef(getBookmarkId("3.8  Technologies Used"), false, 1200)] }),
      new TableRow({ children: [tc("3.9  Hardware and Software Requirements", false, CW - 1200), tcPageRef(getBookmarkId("3.9  Hardware and Software Requirements"), false, 1200)] }),
      new TableRow({ children: [tc("3.10 Summary", false, CW - 1200), tcPageRef(getBookmarkId("3.10 Summary"), false, 1200)] }),
      new TableRow({ children: [tc("CHAPTER 4: IMPLEMENTATION AND RESULTS", true, CW - 1200), tcPageRef(getBookmarkId("CHAPTER 4: IMPLEMENTATION AND RESULTS"), true, 1200)] }),
      new TableRow({ children: [tc("4.1  Introduction", false, CW - 1200), tcPageRef(getBookmarkId("4.1  Introduction"), false, 1200)] }),
      new TableRow({ children: [tc("4.2  Experimental Setup", false, CW - 1200), tcPageRef(getBookmarkId("4.2  Experimental Setup"), false, 1200)] }),
      new TableRow({ children: [tc("4.3  Implementation Details", false, CW - 1200), tcPageRef(getBookmarkId("4.3  Implementation Details"), false, 1200)] }),
      new TableRow({ children: [tc("4.4  Experimental Screenshots", false, CW - 1200), tcPageRef(getBookmarkId("4.4  Experimental Screenshots"), false, 1200)] }),
      new TableRow({ children: [tc("4.5  Performance Evaluation", false, CW - 1200), tcPageRef(getBookmarkId("4.5  Performance Evaluation"), false, 1200)] }),
      new TableRow({ children: [tc("4.6  Discussion of Results", false, CW - 1200), tcPageRef(getBookmarkId("4.6  Discussion of Results"), false, 1200)] }),
      new TableRow({ children: [tc("4.7  Summary", false, CW - 1200), tcPageRef(getBookmarkId("4.7  Summary"), false, 1200)] }),
      new TableRow({ children: [tc("CHAPTER 5: CONCLUSION AND FUTURE WORK", true, CW - 1200), tcPageRef(getBookmarkId("CHAPTER 5: CONCLUSION AND FUTURE WORK"), true, 1200)] }),
      new TableRow({ children: [tc("5.1  Conclusion", false, CW - 1200), tcPageRef(getBookmarkId("5.1  Conclusion"), false, 1200)] }),
      new TableRow({ children: [tc("5.2  Contributions", false, CW - 1200), tcPageRef(getBookmarkId("5.2  Contributions"), false, 1200)] }),
      new TableRow({ children: [tc("5.3  Limitations", false, CW - 1200), tcPageRef(getBookmarkId("5.3  Limitations"), false, 1200)] }),
      new TableRow({ children: [tc("5.4  Future Scope", false, CW - 1200), tcPageRef(getBookmarkId("5.4  Future Scope"), false, 1200)] }),
      new TableRow({ children: [tc("5.5  Summary", false, CW - 1200), tcPageRef(getBookmarkId("5.5  Summary"), false, 1200)] }),
      new TableRow({ children: [tc("REFERENCES", true, CW - 1200), tcPageRef(getBookmarkId("REFERENCES"), true, 1200)] }),
    ]
  }),
  pb(),
  ...
  new Table({
    ...
    rows: [
      ...
      new TableRow({ children: [tcC("3.1", false, 1000), tc("Overall Architecture of the EMARL-SAR Framework", false, CW - 2200), tcPageRef("fig_3_1", false, 1200)] }),
      new TableRow({ children: [tcC("3.2", false, 1000), tc("Two-Level Hierarchical Policy Structure", false, CW - 2200), tcPageRef("fig_3_2", false, 1200)] }),
      new TableRow({ children: [tcC("3.3", false, 1000), tc("Explainability Engine Workflow", false, CW - 2200), tcPageRef("fig_3_3", false, 1200)] }),
  ...
  new Table({
    ...
    rows: [
      ...
      new TableRow({ children: [tcC("2.1", false, 1200), tc("Comparative Study of Existing MARL, HRL, and XAI Methods", false, CW - 2400), tcPageRef("tbl_table_2_1", false, 1200)] }),
      new TableRow({ children: [tcC("3.1", false, 1200), tc("Software Requirements", false, CW - 2400), tcPageRef("tbl_table_3_1", false, 1200)] }),
      new TableRow({ children: [tcC("3.2", false, 1200), tc("Hardware Requirements", false, CW - 2400), tcPageRef("tbl_table_3_2", false, 1200)] }),
      new TableRow({ children: [tcC("4.1", false, 1200), tc("Hyperparameter Configuration", false, CW - 2400), tcPageRef("tbl_table_4_1", false, 1200)] }),
      new TableRow({ children: [tcC("4.2", false, 1200), tc("Environment Configuration Parameters", false, CW - 2400), tcPageRef("tbl_table_4_2", false, 1200)] }),
      new TableRow({ children: [tcC("4.3", false, 1200), tc("Performance Metrics Comparison — EMARL-SAR vs Flat DQN Baseline", false, CW - 2400), tcPageRef("tbl_table_4_3", false, 1200)] }),
    ]
  }),
  pb(),
...
const ch1 = [
  ...
  cpCite("The objectives of this project are:", [9, 12]),
  nl("To design and implement a two-level hierarchical reinforcement learning framework for cooperative multi-agent search and rescue in a dynamic grid environment.", 2),
  nl("To integrate a real-time Explainability Engine that generates human-interpretable justifications and gradient-based saliency attributions for all agent decisions.", 2),
  nl("To evaluate the framework performance through rescue success metrics and comparative analysis against non-hierarchical baselines.", 2),

  secHd("1.7 Scope of the Work"),
  cpCite("The scope of this project encompasses the complete design, implementation, training, and experimental evaluation of the EMARL-SAR framework within a simulated 15×15 dynamic grid environment. The framework supports a heterogeneous team of three agents — two reconnaissance drones (A1, A2) and one rescue rover (A3) — coordinating to locate and rescue three victims distributed across the grid while navigating around ten debris obstacles and two initial fire seed locations that spread stochastically every seven timesteps with an 18% per-cell spread probability.", [11, 14]),

  cpCite("The proposed system includes:", [9, 12]),
  ...
  cpCite("The proposed system is capable of:", [9, 12]),
  ...
  cpCite("The following aspects are explicitly outside the scope of this project and are identified as future work:", [9, 12]),
...
const ch3 = [
  ...
  el(),
  cpTableCaption("Table 3.1", "Software Requirements for the EMARL-SAR Framework"),
  el(),
...
const ch4 = [
  ...
  el(),
  cpTableCaption("Table 4.1", "Hyperparameter Configuration for EMARL-SAR Training"),
...
const refEntry = (text) => {
  ...
  const children = [new TextRun({ text, font: F, size: SZ.content })];
  return new Paragraph({
    children: num ? [new Bookmark({ id: `ref_${num}`, children })] : children,
    spacing: { after: 160, ...LS },
    alignment: AlignmentType.JUSTIFIED
  });
};
